...
class TimelineEventsPostgresIntegrationTests {
    ...
    void testCreateTutorial_ShouldRecordCreatedEventWithFullAttributes() throws Exception {
        ...
        assertEquals(TimelineEventType.CREATED, event.getEventType(), "Event type should be CREATED");
    ...
    void testUpdateTutorial_ShouldRecordUpdatedEventWithDiffAttributes() throws Exception {
        ...
        TimeLineEvent updateEvent = events.stream()
                .filter(e -> e.getEventType() == TimelineEventType.UPDATED)
                .findFirst()
                .orElseThrow(() -> new AssertionError("UPDATE event not found"));

        assertEquals(TimelineEventType.UPDATED, updateEvent.getEventType(), "Event type should be UPDATED");
    ...
    void testDeleteTutorial_ShouldRecordDeletedEventWithEmptyAttributes() throws Exception {
        ...
        TimeLineEvent deleteEvent = events.stream()
                .filter(e -> e.getEventType() == TimelineEventType.DELETED)
                .findFirst()
                .orElseThrow(() -> new AssertionError("DELETE event not found"));

        assertEquals(TimelineEventType.DELETED, deleteEvent.getEventType(), "Event type should be DELETED");
    ...
    void testCompleteLifecycle_ShouldRecordAllEvents() throws Exception {
        ...
        assertEquals(TimelineEventType.CREATED, events.get(0).getEventType(), "First event should be CREATED");
        assertEquals(TimelineEventType.UPDATED, events.get(1).getEventType(), "Second event should be UPDATED");
        assertEquals(TimelineEventType.DELETED, events.get(2).getEventType(), "Third event should be DELETED");
