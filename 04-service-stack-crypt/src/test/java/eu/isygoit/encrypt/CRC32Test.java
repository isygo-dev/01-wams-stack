...
class CRC32Test {
    ...
    void testCalculateByteArray() {
        ...
        assertEquals(0xFEAAE43F, CRC32.calculate(data1));  // Expected CRC-32-ANSI for {0x12, 0x34, 0x56, 0x78}
        assertEquals(0x810E88FF, CRC32.calculate(data2));  // Expected CRC for empty data {0x00, 0x00, 0x00, 0x00}
        assertEquals(0xFFFFFFFF, CRC32.calculate(data3));  // Expected CRC-32-ANSI for {0xFF, 0xFF, 0xFF, 0xFF}
    }
    ...
    void testCalculateFile(@TempDir File tempDir) throws IOException {
        ...
        // Compute CRC32 from file and compare with expected value
        int expectedCrc = 0xFEAAE43F;  // Precomputed CRC-32-ANSI for {0x12, 0x34, 0x56, 0x78}
        assertEquals(expectedCrc, CRC32.calculate(tempFile));
    }
    ...
    void testCalculateFileWithMock() throws IOException {
        ...
        try (MockedStatic<Files> mockedFiles = Mockito.mockStatic(Files.class)) {
            ...
            int expectedCrc = 0xFEAAE43F;  // Precomputed CRC-32-ANSI for {0x12, 0x34, 0x56, 0x78}
            assertEquals(expectedCrc, CRC32.calculate(mockFile));
